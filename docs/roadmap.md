...
| 4 : IA et outillage | **Serveur MCP OpenFlow** et **WebMCP** (18 outils : pages, sections, style, thème, médias, publication ; brouillons uniquement) : **réalisés**, voir [assistant-ia.md](assistant-ia.md). Reste : assistant intégré à l'admin (API Claude dans une Cloud Function, affichage des différences), OAuth pour les connecteurs MCP, `openflow check --against-live` (OF-202) et migrations de schéma, demandes de modification transmises à Claude Code | En partie réalisée |
...
- Ces mêmes outils sont exposés par le serveur MCP OpenFlow (réalisé : `openflowMcp`, voir
