...
9. Styles : classes Tailwind et jetons du thème déclarés dans `app/globals.css` (`@theme` : `ink`, `paper`, `cobalt`,
   `flame`, `graphite`, `line`). N'écris pas de couleur en dur quand un jeton existe. Direction artistique : `DESIGN.md`.
   Le propriétaire peut changer ces jetons (`theme` dans la config : garde `value` identique à `globals.css`) et le
   style de chaque section (onglet « Style », prop réservée `_style`) : pas de `!important`, une seule racine par section.
